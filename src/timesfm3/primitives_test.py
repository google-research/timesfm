...
class UtilTest(unittest.TestCase):
  ...
  def test_update_running_stats_multiple_batches_no_mask(self):
    ...
    n2, mu2, sigma2 = torch_util.update_running_stats(
        n1, mu1, sigma1, x2, mask2
    )
  ...
  def test_revin_no_reverse(self):
    ...
    np.testing.assert_allclose(
        normalized_x.numpy(), expected.numpy(), atol=1e-6
    )
  ...
  def test_revin_reverse(self):
    ...
    np.testing.assert_allclose(
        denormalized_x.numpy(), expected.numpy(), atol=1e-6
    )
  ...
  def test_get_output_patch_via_roll(self):
    ...
    np.testing.assert_allclose(
        output.numpy(), expected_output.numpy(), atol=1e-6
    )
  ...
  def test_get_running_stats_single_segment(self):
    ...
    _, mu, sigma = torch_util.get_running_stats(
        values, masks, segment_ids=segment_ids
    )
  ...
  def test_get_running_stats_multiple_segments(self):
    values = torch.tensor(
        [[[[1, 3], [10, 10], [5, 7], [4, 6]]]], dtype=torch.float32
    )
    masks = torch.zeros_like(values, dtype=torch.bool)
  ...
  def test_get_running_stats_with_masks(self):
    values = torch.tensor(
        [[[[1, 10, 3, 0], [100, 5, 200, 7]]]], dtype=torch.float32
    )
    masks = torch.tensor(
