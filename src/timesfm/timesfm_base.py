...
def freq_map(freq: str):
    ...
    if freq.endswith("MS"):
        ...
        return 0
    elif (freq.endswith(("W", "M")) or freq.startswith("W-")
          or (freq.startswith("M") and len(freq) == 2)):
        return 1
    elif (freq.endswith(("Y", "Q", "A")) or freq.startswith("Y-")
          or freq.startswith("Q-") or freq.startswith("A-")):
        return 2
    else:
...
def _normalize(batch):
    stats = [(np.mean(x), np.where((w := np.std(x)) > _TOL, w, 1.0))
             for x in batch]
  ...
  """
    ...
    """
        ...
        for i, ts in enumerate(inputs):
            ...
            padding = np.zeros(shape=(input_len + self.horizon_len, ),
                               dtype=float)
            if input_len < self.context_len:
                num_front_pad = self.context_len - input_len
                ts = np.concatenate(
                    [np.zeros(shape=(num_front_pad, ), dtype=float), ts],
                    axis=0)
                padding = np.concatenate(
                    [np.ones(shape=(num_front_pad, ), dtype=float), padding],
                    axis=0)
    ...
    def forecast_with_covariates(
        ...
        dynamic_categorical_covariates: (
            dict[str, Sequence[Sequence[Category]]] | None) = None,
    ...
    Args:
      ...
      xreg_mode: one of "xreg + timesfm" or "timesfm + xreg". "xreg + timesfm"
        fits a model on the targets then forecasts on the residuals via TimesFM. "timesfm + xreg"
        fits a model on the residuals of the TimesFM forecast.
    ...
    """
        ...
        for i, input_ts in enumerate(inputs):
            ...
            if xreg_mode == "timesfm + xreg":
                # For fitting residuals, no xreg for the first patch (used as TimesFM context).
            ...
            if dynamic_numerical_covariates:
                test_lens.append(
                    len(list(dynamic_numerical_covariates.values())[0][i]) -
                    input_len)
        ...
        for covariates, train_covariates, test_covariates in (
            ...
            for covariate_name, covariate_values in covariates.items():
                for input_len, train_len, covariate_value in zip(
                    ...
                    test_covariates[covariate_name].append(
        ...
        if xreg_mode == "timesfm + xreg":
            ...
            targets = [(np.array(input_ts)[-train_len:] -
                        mean_output[(self._horizon_start -
                                     train_len):self._horizon_start])
            ...
            outputs = [(mean_output[self._horizon_start:(self._horizon_start +
                                                         test_len)] + xreg)
                       for mean_output, test_len, xreg in zip(
                           mean_outputs, test_lens, xregs)]
            ...
            mean_outputs, _ = self.forecast(
                [
                    target - xreg_on_context for target, xreg_on_context in
                    zip(targets, xregs_on_context)
                ],
                freq,
                window_size,
                forecast_context_len,
    ...
    """
        if not ("unique_id" in inputs.columns and "ds" in inputs.columns
        ...
        fcst_df[model_name] = full_forecast[:, 0:self.horizon_len,
                                            0].reshape(-1, 1)
